...
// The body field is `text`, although the API's error for a missing one says
// "content_text is required" — sending content_text is exactly what triggers
// it. `to` must be E.164, so the stored bare digits get their "+".
async function sendSession(waId, text) {
  const { res, body } = await wacrm('/messages', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ to: `+${String(waId).replace(/\D/g, '')}`, type: 'text', text }),
  });
