...
// the cache is keyed by that variable rather than being a single client — a
// campaign that shares another's database (prep) shares its client too.
...
function urlFor(c) {
  const url =
    process.env[c.urlEnv] ||
    (c.urlEnv === campaign(DEFAULT_CAMPAIGN).urlEnv ? process.env.MONGODBURL : '');
  if (!url) {
    throw new Error(
...
function connect(c) {
  ...
  if (!cache.clients[slot]) {
    ...
    cache.clients[slot] = client.connect().catch((err) => {
      ...
      cache.clients[slot] = null;
      throw err;
    });
  }

  return cache.clients[slot];
}

// The one collection each campaign reads and writes — button_replies unless the
// campaign names its own. Undefined DB name means "use the database named in
// the connection string".
async function leadsCollection(campaignKey = DEFAULT_CAMPAIGN) {
  return campaignCollection(campaignKey, campaign(campaignKey).collection || COLLECTION);
}

// Any other collection in a campaign's database — the CA Guru webhook keeps
// its raw payloads beside its leads.
async function campaignCollection(campaignKey, name) {
  const c = campaign(campaignKey);
  const client = await connect(c);
  return client.db(process.env[c.dbEnv] || undefined).collection(name);
}
...
module.exports = { leadsCollection, campaignCollection, stateCollection, COLLECTION };
