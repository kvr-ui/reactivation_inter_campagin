...
const { watiCampaigns, normalize } = require('./campaigns');

// A lead whose whole message is one of these has opted out. Matched on the
// entire message, not a substring, so "don't stop" or "stop by tomorrow" are
// ordinary replies. Trailing punctuation is ignored ("Stop." counts).
...
async function recordMessage(payload) {
  ...
  // A STOP moves the lead to the chat's STOP column. It stays there: nothing
  // here ever clears the flag.
  if (!outbound && isStop(preview)) {
    update.$set.stopped = true;
    update.$set.stoppedAt = at;
  }

  let touched = 0;

  await Promise.all(
    watiCampaigns().map(async ({ key }) => {
...
// Opening the conversation is what clears the badge — and the "new" on a
// calculator click.
async function markRead(waId, campaignKey) {
  const col = await leadsCollection(campaignKey);
  await col.updateOne({ waId }, { $set: { unread: 0, calcNew: false, readAt: new Date() } });
}

module.exports = { recordMessage, markRead, previewOf, timeOf, isStop };
