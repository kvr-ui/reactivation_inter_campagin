...
// Only leads who click this exact button get stored; everything else is dropped.
const TARGET_REPLY = 'LAST ATTEMPT';
...
async function saveReply(payload) {
  ...
  // Ignore every other button — spacing and casing from WATI shouldn't matter.
  if (buttonReply.trim().toUpperCase() !== TARGET_REPLY) {
    console.log(`--- skipped, not "${TARGET_REPLY}" --- got "${buttonReply}"`);
    return null;
  }
  ...
  try {
    ...
    const result = await replies.updateOne(
      ...
      {
        $setOnInsert: {
          buttonReply: TARGET_REPLY, // normalized: only exact matches reach here
          receivedAt: now,
        },
        $set: {
          senderName,
    ...
    if (result.upsertedId) {
      console.log(`--- saved to mongo --- _id=${result.upsertedId}`);
      console.log(JSON.stringify({ waId, senderName, buttonReply }));
      return result.upsertedId;
    }

    console.log(`--- duplicate lead, row refreshed --- ${waId}`);
    return null;
  } catch (err) {
    if (err.code === 11000) {
      // Two webhooks for the same new lead racing each other; the other won.
      console.log(`--- duplicate lead, already stored --- ${waId}`);
      return null;
    }
    throw err;
  }
...
async function deleteLead(waId) {
  const replies = await leadsCollection();
...
async function deleteLeads(waIds) {
  const replies = await leadsCollection();
  const result = await replies.deleteMany({ waId: { $in: waIds } });
  return result.deletedCount;
}

async function setContacted(waId, contacted) {
  const replies = await leadsCollection();
  const result = await replies.updateOne(
    { waId },
    {
